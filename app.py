...
def index():
    ...
    perfil = obtener_perfil_usuario(session['user'])
    
    banner = None
    if perfil['estado'] == "Gracia":
        banner = ("⚠️ Período de Gracia: Tu suscripción venció ayer. Regulariza tu pago.", "alert-warning")
...
def chat():
    perfil = obtener_perfil_usuario(session.get('user'))
    if perfil['estado'] == "Vencido": 
        return jsonify({"reply": "Tu suscripción ha expirado. Por favor, renueva tu plan."})
    
    # ... (Resto de tu lógica de Mistral y Gráficos enviada ayer)
    return jsonify({"reply": "Análisis en proceso...", "chart": None})
